...
def image_label(im_fn: str, text_polys: np.ndarray, text_tags: list, n: int, m: float, input_size: int,
    ...
    #imgs = [im, score_maps.transpose((1, 2, 0)),training_mask]
    return imgs[0], imgs[1].transpose((2, 0, 1)), imgs[2]#im,score_maps,training_mask#


class MyDataset(data.Dataset):
    def __init__(self, data_dir, data_shape: int = 640, n=3, m=0.5, transform=None, target_transform=None):
...
if __name__ == "__main__":
    ...
    train_root= "/data/tips/train"
    train_data = MyDataset(train_root, data_shape=640, n=3, m=0.5,
    ...
    for i, (img, label, mask) in enumerate(train_loader):
        ...
        mask_contours = get_the_mask_contours(mask)
        seg_contours = get_the_segment_contours(label[0][0])
        img_raw = get_the_cv2_image(img)
        cv2.drawContours(img_raw, mask_contours, -1, (0, 0, 255), 2)
        cv2.drawContours(img_raw, seg_contours,-1, (0,255,0),1)
